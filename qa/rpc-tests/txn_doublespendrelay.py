...
class DoubleSpendRelay(BitcoinTestFramework):
    ...
    def setup_network(self):
        ...
        self.nodes = []
        for i in range(0,4):
            self.nodes.append(start_node(i, self.options.tmpdir, ["-debug", "-replacebyss=1"]))
    ...
    def run_test(self):
        ...
        # Restart nodes[1]
        nodes[1] = start_node(1, self.options.tmpdir, ["-debug", "-replacebyss=1"])
        ...
        # Test 4: txid5 respends non-SS txid4
        # txid5 should be relayed and replace txid4 in mempools
        stop_node(nodes[1], 1)

        # txid4 is a new first-spend. It is non-SS because size > 2Kb
        amount = Decimal("99")
        ...
        outputs[nodes[3].getnewaddress()] = amount
        for i in range(85):
            outputs[nodes[3].getnewaddress()] = 0
        rawtx = nodes[0].createrawtransaction(tx4_inputs, outputs)
        signed = nodes[0].signrawtransaction(rawtx)
