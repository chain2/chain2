...
bool CTxMemPoolEntry::IsLiveSS() const
{
    if (!IsSuperStandardTx(tx, hadNoDependencies, nTxSize))
        return false;

    AssertLockHeld(cs_main);

    if (chainActive.Height() < nHeight + 2)
        return true;
...
void CTxMemPool::removeForReorg(const CCoinsViewCache *pcoins, unsigned int nMemPoolHeight)
{
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        if (!CheckFinalTx(tx) || !CheckSequenceLocks(tx, &lp, validLP)) {
